"""Alembic environment for the EDIS decision service (L4).
...
SQLAlchemy engine (asyncpg) via :func:`run_migrations_online`; no engine is
created at import time. There is no shared ``target_metadata`` because these
migrations are hand-authored raw DDL (the recommendation / lifecycle /
outcome_report / calibration_prior tables are owned here, not reflected from ORM
models -- the ORM models in ``decision_engine.models`` must MATCH this schema).
"""
